...
def process_files_in_config(config_file):
    with open ('output.json', 'w') as f:   
        ...
        f.write('{"code":[17,218,39,0,197,16,0,0,17,218,39,0,66,170,35,0,17,218,39,0,0,40,36,0,0,0,0,6,96,0,0,197,0,8,0],"cmdGr":[')
                    ...
                    for hex_pos, line_nums in unique_lines_by_hex_pos.items():
                        for key, (filtered_content, file_path) in filtered_files.items():
                            if key in config[section]:
                                ...
                                if file_name not in [item["name"] for item in output[section]["cmd"]]:

...
def process_files_in_config(config_file):
    ...
    output = {}  # Dictionary to store output for each section
    for section in config.sections():
        section_output = {}
        section_output["id"] = section  # Add "id" field with section name
        section_output["cmd"] = []  # Initialize "cmd" field as a list
        ...
        if section != "temp":
            ...
            if unique_lines_by_hex_pos:
                section_output = {"id": section, "cmd": []}  # Change to square brackets
                for hex_pos, line_nums in unique_lines_by_hex_pos.items():
                    for key, (filtered_content, file_path) in filtered_files.items():
                        if key in config[section]:
                            ...
                            cmd_entry = {key: [[hex_pos, 15, 12], [hex_pos, converted_value, 13]]}  # Create dictionary entry with key and value
                            if file_name not in section_output["cmd"]:
                                section_output["cmd"].append({file_name: [cmd_entry]})  # Append file_name as a dictionary with a list containing cmd_entry
                            else:
                                section_output["cmd"][file_name].append(cmd_entry)  # Append cmd_entry to existing file_name entry
    ...
    section_output["tCod"] = [tdis, mintemp, maxtemp]
    section_output["tDis"] = [tinc, tfirst, tlast]
    section_output["tAdd"] = thex
    section_output["tUnit"] = tunit
    section_output["cInf"] = values
    section_output["cPre"] = pre
    section_output["cConf"] = conf
    section_output["cId"] = id  
